...
import com.project.nexstay.entity.*;
...
import com.project.nexstay.repository.*;
import com.project.nexstay.strategy.PricingService;
...
public class BookingServiceImpl implements BookingService{
    ...
    public BookingDto initiateBooking(BookingRequest bookingRequest) {
        ...
        Hotel hotel=hotelRepository.findById(bookingRequest.getHotelId()).orElseThrow(()->new ResourceNotFoundException("Hotel with ID: "+bookingRequest.getHotelId()+" does not exists!"));
        Room room=roomRepository.findById(bookingRequest.getRoomId()).orElseThrow(()->new ResourceNotFoundException("Room with ID: "+bookingRequest.getRoomId()+" does not exists!"));
        ...
        if(inventoryList.size()!=daysCount){
            throw new IllegalStateException("Room is not available during these interval of days");
        }
        ...
        BigDecimal totalPrice=priceForOneRoom.multiply(new BigDecimal(bookingRequest.getRoomsCount()));
    ...
    public String initiatePayments(Long bookingId) {
        Booking booking=bookingRepository.findById(bookingId).orElseThrow(()->new ResourceNotFoundException("Bokking with id: "+bookingId+" does not exists!"));
        User user=getCurrentUser();
    ...
    public void capturePayment(Event event) {
        if("checkout.session.completed".equals(event.getType())){
          Session session=(Session)event.getDataObjectDeserializer().getObject().orElse(null);
            ...
            log.warn("Unhandled event of type: {}",event.getType());
        }
    }
    ...
    public void cancelBooking(Long bookingId) {
        ...
        if(booking.getBookingStatus()!=BookingStatus.CONFIRMED){
            throw new IllegalStateException("Booking status is not Confirmed, cannot cancel this booking");
        }
    ...
    public List<BookingDto> getAllBookingsByHotelId(Long hotelId) {
        ...
        log.info("Getting all the bookings for the hotel with id: {}",hotelId);
        if(!hotel.getOwner().equals(user)){
            throw new AccessDeniedException("Not authorised to perform this operation");
        }
